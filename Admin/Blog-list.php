...
// ---- Fetch all blogs ----
$blogs = array();
$result = mysqli_query($con, "select * from blog order by id desc") or die(mysqli_error($con));
...
$latest = '';
foreach ($blogs as $b) {
    if ($b['img'] !== '' && $b['img'] !== 'noimg.jpg') { $with_images++; }
    if ($b['blog_date'] > $latest) { $latest = $b['blog_date']; }
}
$latest_txt = ($latest && $latest !== '0000-00-00') ? date('d M Y', strtotime($latest)) : '—';
?>
<!doctype html>
...
<head>
    ...
    <style>
        ...
        .prod-thumb{width:64px;height:48px;object-fit:cover;border-radius:9px;border:1px solid #eef0f4;background:#fff;box-shadow:0 2px 6px rgba(0,0,0,.05);}
        ...
        .prod-thumb:hover{transform:scale(1.1);box-shadow:0 6px 18px rgba(0,0,0,.18);}
        ...
        @media (prefers-reduced-motion: reduce){ .stat-card,.card.shadow-sm,#myDataTable tbody tr{animation:none;} .prod-thumb:hover,.btn-primary:hover{transform:none;} }
    </style>
</head>
<body>
                    ...
                    <div class="row align-items-center">
                        <div class="border-0 mb-3">
                            <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                                ...
                                <a href="blog-add.php" class="btn btn-primary d-inline-flex align-items-center">
                                    <i class="icofont-plus me-2"></i> Add Blog
                                </a>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-6 col-lg-4">
                            <div class="card stat-card bg-white">
                                <div class="card-body d-flex align-items-center">
                                    <div class="stat-icon" style="background:#eef3ff;color:#3b6fe0;"><i class="icofont-newspaper"></i></div>
                                    <div class="ms-3"><div class="stat-value"><?php echo $total_blogs; ?></div><div class="stat-label text-muted">Total Blogs</div></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-4">
                            <div class="card stat-card bg-white">
                                <div class="card-body d-flex align-items-center">
                                    ...
                                    <div class="ms-3"><div class="stat-value"><?php echo $with_images; ?></div><div class="stat-label text-muted">With Image</div></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-4">
                            <div class="card stat-card bg-white">
                                <div class="card-body d-flex align-items-center">
                                    <div class="stat-icon" style="background:#fff5e6;color:#d98a00;"><i class="icofont-calendar"></i></div>
                                    <div class="ms-3"><div class="stat-value" style="font-size:18px;"><?php echo $latest_txt; ?></div><div class="stat-label text-muted">Latest Post</div></div>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table id="myDataTable" class="table table-hover align-middle mb-0" style="width: 100%;">
                                            ...
                                            <tbody>
                                            <?php $count = 0; foreach ($blogs as $row):
                                                $count++;
                                                $id    = $row['id'];
                                                $title = $row['blog_title'];
                                                $bdate = $row['blog_date'];
                                                $img   = $row['img'];
                                                $src   = ($img !== '' && $img !== 'noimg.jpg') ? 'blog_image/' . $img : 'blog_image/noimg.jpg';
                                                $short = mb_strimwidth(strip_tags($row['blog_description']), 0, 80, '…');
                                            ?>
                                                <tr id="delete<?php echo $id;?>">
                                                    ...
                                                    <td class="text-end">
                                                        ...
                                                        <button type="button" onclick="deleteblog(<?php echo $id;?>)" class="act-btn act-del deleterow" title="Delete"><i class="icofont-ui-delete"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                            </tbody>
    ...
    <script>
        ...
        <?php if ($_GET['updated'] === '1'): ?>
            toastr["success"]("Blog updated successfully!", "Saved");
        <?php else: ?>
            toastr["error"]("Could not update the blog. Please try again.", "Failed");
        <?php endif; ?>
    </script>
    <?php endif; ?>
    ...
    <script>
        toastr.options = { "closeButton": true, "progressBar": true, "positionClass": "toast-top-right", "timeOut": "4000" };
        <?php if ($_GET['added'] === '1'): ?>
            toastr["success"]("Blog added successfully!", "Added");
        <?php else: ?>
            toastr["error"]("Could not add the blog. Please try again.", "Failed");
        <?php endif; ?>
    </script>
    <?php endif; ?>
